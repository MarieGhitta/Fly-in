...
class Parser:
    ...
    def _parse_zone_line(
        ...
        return count_start_hub, count_end_hub
            ...
            for key, value in metadata_dct.items():
                if key == "max_link_capacity":
                    try:
                        ...
                        raise ValueError(f"ERROR line {number_line}: "
                                         "max_link_capacity value should "
                                         "be an integer.")
    ...
    def _parse_drones_line(self, line: str, number_line: int) -> None:
        if ":" not in line:
            raise ValueError(f"ERROR line {number_line}: "
                             "format is not correct. "
                             "Should be like this: "
            ...
            with open(self.filename, 'r') as file:
                for number_line, line in enumerate(file, start=1):
                    ...
                    if not check_first_line:
                        if not stripped_line.startswith('nb_drones'):
                            raise ValueError(f"ERROR line {number_line}: "
                                             "the first non-empty line must "
                                             "define the number of drones.")
                        self._parse_drones_line(stripped_line, number_line)
                    ...
                    if (stripped_line.startswith("hub")
                       ...
                       or stripped_line.startswith("end_hub")):
                        count_start_hub, count_end_hub = self._parse_zone_line(
                                                            line,
                                                            stripped_line,
                                                            number_line,
                                                            lst_zone,
                    ...
                    elif stripped_line.startswith("connection"):
                ...
                for conn in self.connections:
                    if (conn["from"] not in self.check_name_zone
                       or conn["to"] not in self.check_name_zone):
                        raise ValueError("ERROR: connection references "
                                         "unknown zone.")
